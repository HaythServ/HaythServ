...
class anticheat
{
     ...
     */
    /* TODO: FIX with other insta modes, so disactivated for now.
	...
	if (server::cginsta) {
            bool correct_gun = gunselect == GUN_CG || gunselect == GUN_FIST;
	}else{
	    if (server::sginsta) {
                bool correct_gun = gunselect == GUN_SG || gunselect == GUN_FIST;
	    }else{
		if (server::rlinsta) {
            	    bool correct_gun = gunselect == GUN_RL || gunselect == GUN_FIST;
		}else{
		    if (server::glinsta) {
			bool correct_gun = gunselect == GUN_GL || gunselect == GUN_FIST;
		    }else{
